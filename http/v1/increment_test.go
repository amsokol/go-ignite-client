...
func Test_client_Increment(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotValue, gotNodeID, gotToken, err := tt.c.Increment(tt.args.cache, tt.args.key, tt.args.init, tt.args.delta)
			if (err != nil) != tt.wantErr {
				t.Errorf("client.Increment() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			t.Log("")
			t.Logf("http.v1.client.Increment returned for '%s':", tt.name)
