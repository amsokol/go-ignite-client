...
func TestCommands_Prepend(t *testing.T) {
	t.Log("")
	t.Log("Preparing test data for 'TestCommands_Prepend'...")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("Commands.Prepend() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotOk != tt.wantOk {
				t.Errorf("Commands.Prepend() gotOk = %v, want %v", gotOk, tt.wantOk)
			}
		})
	}
}
