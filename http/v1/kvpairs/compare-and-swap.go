...
func (p *Commands) CompareAndSwap(e exec.Executer, cache string, key string, val string, val2 string, destID string) (ok bool, nodeID string, token string, err error) {
	v := url.Values{}
	v.Add("cmd", "cas")
	if len(cache) > 0 {
